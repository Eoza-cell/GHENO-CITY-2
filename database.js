...
const { WADriver } = require('@journeyapps/wa-sqlite');
const { vehicles } = require('./data');

const sequelize = new Sequelize({
  dialect: 'sqlite',
  dialectModule: WADriver,
...
async function setupDatabase() {
  try {
    ...
    if (vehicleCount === 0) {
      console.log('La table des véhicules est vide. Initialisation à partir de data.js...');
      await Vehicle.bulkCreate(vehicles);
      console.log(`${vehicles.length} véhicules ont été ajoutés à la base de données.`);
    }
  } catch (error) {
