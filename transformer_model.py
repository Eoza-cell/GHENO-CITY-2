...
def main():
    if len(sys.argv) < 3:
        system_prompt = "MJ D'AFTER THE REBIRTH (ATR) RPG"
    ...
    # Hugging Face Transformers Local Gemma / Qwen Model Pipeline
    try:
        ...
        formatted_messages = [
            {"role": "system", "content": "Tu es le Meneur de Jeu (MJ) d'After the Rebirth (ATR). Narre en français de manière très vivante, fluide et immersive (style anime Shonen/Seinen). Ne parle pas de toi-même en dehors de l'histoire."},
            {"role": "user", "content": clean_action}
        ]

        result = pipe(formatted_messages, max_new_tokens=280, temperature=0.7)
    ...
    # High quality Hugging Face Transformers Fallback Generator
