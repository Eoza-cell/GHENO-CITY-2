...
async function generateShopImage(title, items) {
    ...
    items.forEach((item, i) => {
        ...
        itemsSvg += `
            <g transform="translate(40, ${y})">
                ...
                <text x="20" y="35" font-family="Arial" font-size="20" font-weight="bold" fill="${rarityColor}">${item.name.toUpperCase()}</text>
                <text x="20" y="65" font-family="Arial" font-size="14" fill="#aaa">${item.description.substring(0, 70)}...</text>
    ...
    const svg = `
        <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
            ...
            <text x="40" y="70" font-family="Arial" font-size="42" font-weight="900" fill="#ffd700">${title}</text>
    ...
    const outputPath = path.join(__dirname, 'assets', `shop_${Date.now()}.png`);
